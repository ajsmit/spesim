...
p_voronoi_diag <- ggplot() +
  geom_sf(data = domain, fill = NA, colour = "#22223b", linewidth = 0.7) +
  geom_sf(data = vor_cells, fill = NA, colour = "grey80", linewidth = 0.3) +
  geom_sf(data = q_voronoi, fill = "#4a4e69", alpha = 0.15, colour = "#22223b", linewidth = 0.5) +
  coord_sf(datum = NA) +
  labs(title = "Voronoi diagram (clipped) + selected quadrats") +
  theme_minimal(base_size = 12) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
