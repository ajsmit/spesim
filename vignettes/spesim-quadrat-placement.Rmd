...
Quadrat placement can fail silently in subtle ways when the domain is irregular
(e.g., many candidates fall outside the boundary, or random placement hits an
attempt cap).

spesim attaches a `placement_audit` attribute to the returned quadrats. This is
used by `spesim_audit()`/`audit_sampling_scheme()`, and it is also useful to
inspect directly.
