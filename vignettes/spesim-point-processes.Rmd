...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Point Processes in spesim: Options and Intuition}
...
- **Geyer saturation**: Favour moderate clustering but saturate local counts.
  *Approximation*: Weight by neighbour counts within r, cap at a threshold.
  spesim's fast Geyer engine is **heuristic but internally consistent** — it
  targets a saturation potential based on per-point neighbour counts and is
  **not** a reproduction of the exact **spatstat** Geyer saturation process
  likelihood (see `?simulate_points_geyer_fast` for details).
