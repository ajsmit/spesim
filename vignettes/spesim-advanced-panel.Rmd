...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Inside the Advanced Analysis Panel}
...
- **Method**: `vegan::rarefy()` by site (quadrat) across increasing subsample
  sizes (equivalent to the computation done by `vegan::rarecurve()`, but without
...
- The SAR component uses random permutations internally (`vegan::specaccum(method = "random")`);
  set `set.seed()` beforehand for reproducibility if you re-run it.
- Rarefaction here is deterministic (via `vegan::rarefy()`).
